...
for filename in os.listdir('data/modeling_data/resampled/targets'):
    ...
    # df[['Average Sap Flux']].to_csv('Penman_Monteith/sap_flux_cm3-sec/'+site+'_cm3-sec.csv')  # Saves sap flux in cm3/s for all sites
    ...
    for filename_leaf in os.listdir('data/leaf'):  # Saves the subset of available leaf data, converted to um/sec
        if site in filename_leaf and 'sapf_data' in filename_leaf:
            leaf_df = pd.read_csv('data/leaf/'+site+'_sapf_data.csv')
            leaf_df['Average Sap Flux'] = leaf_df.iloc[:, 2:].mean(axis=1) * 10000 / 3600  # conversion factor included from cm/hr to um/s
            leaf_df[['TIMESTAMP', 'Average Sap Flux']].to_csv('Penman_Monteith/sap_flux_um-sec/'+site+'_um-sec.csv')
